...
'pizza', 'pork_chop', 'poutine', 'prime_rib', 'pulled_pork_sandwich', 'ramen', 'ravioli', 'red_velvet_cake', 'risotto', 'samosa', 'sashimi', 'scallops', 'seaweed_salad', 'shrimp_and_grits', 'spaghetti_bolognese', 'spaghetti_carbonara', 'spring_rolls', 'steak', 'strawberry_shortcake', 'sushi', 'tacos', 'takoyaki', 'tiramisu', 'tuna_tartare', 'waffles']
model = load_model('model4b.10-0.68.hdf5')

def recognize_food(image):
    pic = Image.open(image).resize((299,299)).convert('RGB')
    pic = np.asarray(pic)
    pic = np.expand_dims(pic,0)
    pic = preprocess_input(pic.astype('float32'))
    pr = model.predict(pic)
    arg = np.argmax(pr)
    return classes[arg]